...
const features = [
  {
    ...
    description: 'Every Properties project undergoes rigorous legal checks and verification before listing.',
  },
  {
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-cta-brown" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    description: 'Join thousands of Ugandans pooling resources to make Properties ownership accessible.',
  },
  {
    icon: (
       <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-cta-brown" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; description: string }> = ({ icon, title, description }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 text-left shadow-sm border border-gray-100 dark:border-gray-700 transition-all duration-300 hover:shadow-md hover:-translate-y-1">
    ...
    <h3 className="text-lg font-bold text-text-dark dark:text-white mb-2">{title}</h3>
    <p className="text-slate-gray dark:text-gray-400 text-base leading-relaxed">{description}</p>
  </div>
);
    ...
    <section className="bg-light-bg dark:bg-gray-900 py-20 sm:py-28 transition-colors duration-300">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-text-dark dark:text-white">Why Choose Asset Cycle Africa</h2>
