...
const Logo: React.FC<{ onNavigate: (page: Page) => void }> = ({ onNavigate }) => (
    <div className="flex items-center space-x-2 cursor-pointer" onClick={() => onNavigate(Page.HOME)}>
        ...
        <span className="text-2xl font-bold text-text-dark dark:text-white">Asset Cycle Africa</span>
    </div>
);
            ...
            <div className="max-w-7xl mx-auto bg-white/80 dark:bg-gray-900/90 backdrop-blur-md rounded-full shadow-lg border border-white/30 dark:border-gray-700">
                <div className="flex items-center justify-between h-16 px-6">
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                            ...
                            {NAV_LINKS.map((link) => {
                                ...
                                return (
                                    <a 
                                        ...
                                        className={`font-medium px-4 py-2 rounded-full transition-colors duration-300 ${isActive ? 'bg-gray-200 dark:bg-gray-700 text-text-dark dark:text-white' : 'text-text-dark dark:text-gray-300 hover:bg-black/5 dark:hover:bg-white/10'}`}
                                    >
                                        {link.name}
                                    </a>
                                )
                        ...
                        <Button onClick={() => onNavigate(Page.GET_STARTED)} variant="primary" size="md" className="rounded-lg">Get Started</Button>
                    </div>
                    ...
                    <div className="md:hidden flex items-center space-x-2">
                        ...
                        <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="inline-flex items-center justify-center p-2 rounded-full text-text-dark dark:text-white hover:bg-black/5 dark:hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-text-dark">
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden mt-2 max-w-7xl mx-auto bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-2xl shadow-lg border border-white/30 dark:border-gray-700 overflow-hidden">
                    ...
                    <div className="px-5 py-4">
                        <Button onClick={() => { onNavigate(Page.GET_STARTED); setIsMobileMenuOpen(false); }} variant="primary" className="w-full rounded-lg">Get Started</Button>
                    </div>
                </div>
            )}
        </header>
