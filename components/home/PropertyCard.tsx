...
const PropertyCard: React.FC<PropertyCardProps> = ({ listing, onShowDetails }) => {
  const formatNumber = (num: number) => new Intl.NumberFormat('en-US').format(num);

  return (
    <div 
        className="group bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-xl hover:scale-[1.02] transition-all duration-300 flex flex-col h-full"
    >
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow relative">
        ...
        <div className="flex items-center text-slate-gray dark:text-gray-400 text-sm mb-4 sm:mb-6">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 text-accent-green flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="mt-auto relative lg:h-12 lg:overflow-hidden">
             <div className="flex items-end justify-between mb-4 lg:mb-0 lg:absolute lg:inset-0 lg:transition-all lg:duration-500 lg:ease-in-out lg:transform lg:group-hover:-translate-y-full lg:group-hover:opacity-0">
                 <div>
                    ...
                    <span className="text-xs text-slate-gray dark:text-gray-400 font-medium mt-1 block">/ acre</span>
                 </div>
                 <span className="text-sm font-medium text-text-dark dark:text-white mb-1">{listing.area} acres</span>
             </div>
